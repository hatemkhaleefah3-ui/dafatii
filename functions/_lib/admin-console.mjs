...
async function syncAssignments(db,teacherId,content,now){
  ...
  for(const subject of content.subjects){
    await db.prepare(`INSERT INTO school_teacher_assignments
      ...
      .bind(crypto.randomUUID(),teacherId,subject.id,subject.fameScore,now,now).run();
  }
}
...
async function listTeachers(context){
  ...
  const result=await context.env.DB.prepare(`SELECT p.teacher_user_id, p.image_url, p.content_json, p.updated_at,
...
async function createTeacher(context){
  ...
  await context.env.DB.prepare('INSERT INTO school_teacher_profiles (teacher_user_id,image_url,content_json,created_at,updated_at) VALUES (?,?,?,?,?)')
    .bind(user.id,imageUrl,JSON.stringify(content),now,now).run();
  await syncAssignments(context.env.DB,user.id,content,now);
  const row=await context.env.DB.prepare(`SELECT p.teacher_user_id,p.image_url,p.content_json,p.updated_at,u.email_normalized,u.display_name
...
async function updateTeacher(context,teacherId){
  ...
  const current=await context.env.DB.prepare('SELECT 1 AS present FROM school_teacher_profiles WHERE teacher_user_id=?').bind(teacherId).first();
  ...
  await context.env.DB.prepare('UPDATE school_teacher_profiles SET image_url=?,content_json=?,updated_at=? WHERE teacher_user_id=?')
    .bind(imageUrl,JSON.stringify(content),now,teacherId).run();
