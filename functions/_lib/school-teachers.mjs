...
async function legacyCandidates(db, subject) {
  ...
  for (const row of result.results || []) {
    ...
    const existing = byTeacher.get(row.owner_user_id);
    const fameScore = Number(row.popularity || 0);
    if (!existing || fameScore > existing.fameScore) byTeacher.set(row.owner_user_id, {
...
async function candidatesFor(db, identity, subject) {
  const [formal, legacy] = await Promise.all([formalCandidates(db, identity, subject), legacyCandidates(db, subject)]);
  const merged = new Map();
  for (const teacher of [...legacy, ...formal]) {
    const previous = merged.get(teacher.id);
    merged.set(teacher.id, previous ? {
      ...previous,
      ...teacher,
      ...
      source:teacher.source === 'directory' ? 'directory' : previous.source
    } : teacher);
  }
  return [...merged.values()].sort((a,b) => (b.fameScore - a.fameScore) || (b.selectionCount - a.selectionCount) || a.displayName.localeCompare(b.displayName));
}
...
export async function schoolTeacherCatalog(db, actor) {
  ...
  const selections = await selectionMap(db, actor.id);
  const subjects = [];
  for (const subject of SCHOOL_SUBJECTS) {
    const teachers = await candidatesFor(db, identity, subject);
    subjects.push({ subject, selectedTeacherId:selections.get(subject) || null, teachers });
  }
  return {
