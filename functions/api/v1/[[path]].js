...
import { gradePronunciation, gradeVideoUnderstanding } from '../../_lib/gemini.mjs';
...
async function dispatch(context) {
  ...
  if (method === 'POST' && path === 'language/pronunciation') {
    const input = await readJson(context.request, 4_200_000);
    return ok(await gradePronunciation(context.env, input));
  }
