...
assert.ok(ui.includes("if(isHigher())await withDeadline") && !ui.includes("try{await window.DafatiiCourses?.refresh?.();await determineView();}"), 'school Process 1 must not wait for the Courses API before loading teachers');
...
assert.ok(index.includes('onboarding-flow.js?v=20260918-2') && index.includes('onboarding-flow.css?v=20260918-1') && !index.includes('pre-course.css'), 'only the new full-screen onboarding assets should be active');
