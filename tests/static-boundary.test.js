...
assert.match(index, /content-controls\.css\?v=20260920-3/, 'the unified content control presentation must be loaded');
assert.match(index, /content-controls\.js\?v=20260920-3/, 'the unified content control controller must be loaded');
...
assert.match(index, /app\.js\?v=20260920-5/, 'the verified administrator access build must be loaded');
...
assert.match(index, /calendar\.js\?v=20260919-20/, 'the cleaned calendar interface must be loaded');
assert.match(index, /calendar\.css\?v=20260919-18/, 'retired calendar toolbar styling must be removed from the active asset');
...
assert.match(index, /student-social\.css\?v=20260919-4/, 'the multi-page chat application stylesheet must be loaded');
assert.match(index, /student-social\.js\?v=20260919-2/, 'the multi-page chat application controller must be loaded');
assert.match(index, /advanced-chat\.js\?v=20260919-3/, 'the advanced chat engine must load the multi-page integration build');
assert.match(index, /advanced-chat\.css\?v=20260919-4/, 'the advanced chat engine must load its dedicated shell integration styles');
...
assert.match(quietShell, /shell\.classList\.toggle\('chat-app-host',current\.split\('\/'\)\[0\]==='chat'\)/, 'the shared workspace shell must enter dedicated Chat app mode on Chat list and nested thread routes');
...
assert.ok(lectureMedia.includes('Lecture study tools v3') && lectureMedia.includes("flashcards:{") && lectureMedia.includes("mcqs:{") && lectureMedia.includes("qa:{"), 'lectures must expose dedicated Flashcards, MCQ and Question & Answer modules');
...
assert.ok(lectureMedia.includes("window.addEventListener('dafatii:viewer-study-action',viewerStudyRoute)") && lectureMedia.includes("const actionMap={flashcards:'flashcards',mcqs:'mcqs','question-answer':'qa'}") && lectureMedia.includes('routeTo(subjectId,lecture.id,type)'), 'video and file reader study actions must route to the owning lecture reader pages');
assert.doesNotMatch(lectureMedia, /function cardButtons|function decorateCards|data-lecture-study-launch/, 'study-tool launch buttons must not appear on lecture cards or at subject scope');
assert.ok(lectureMedia.includes("const studyAfter=event.currentTarget.dataset.studyAfter||''") && lectureMedia.includes('form.dataset.studyAfter=type') && lectureMedia.includes('form.requestSubmit()'), 'new-lecture study buttons must save that lecture before opening its lecture-specific creation page');
assert.ok(lectureMedia.includes('context.lecture.studyTools[context.type]') && !lectureMedia.includes('context.subject.studyTools'), 'each study set must be stored on its owning lecture, never on the subject');
...
assert.ok(lectureMediaCss.includes('Lecture study tools v2') && lectureMediaCss.includes('.flashcard-stage') && lectureMediaCss.includes('.mcq-exam-card') && lectureMediaCss.includes('.qa-exam-card') && lectureMediaCss.includes('.lecture-study-manager'), 'the three study pages and their manager must have dedicated responsive styling');
...
assert.match(index, /styles\.css\?v=20260920-5/, 'administrator access styling must be cache-busted');
