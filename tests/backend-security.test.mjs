...
assert.match(apiRouter, /const driveUpload = teacherProfile \|\| chatAttachment \|\| usesDrive\(context\.env\)/, 'Drive upload provider behavior changed');
assert.doesNotMatch(apiRouter, /language-hearing-audio|hearingAudioKey|isR2HearingObject|uploadR2HearingAudio|r2\/hearing-audio|provider: 'r2-proxy'/, 'removed Language Course media backend must stay deleted');
...
assert.doesNotMatch(courseLibrarySource, /purgeRetiredCourseProduct|dafatii:language-learning|dafatii:language-content|dafatii:language-authoring|courseType.*language/i, 'removed Language Course backend must not remain in course services');
