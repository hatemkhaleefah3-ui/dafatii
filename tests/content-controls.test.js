...
assert.ok(css.includes('grid-template-columns:1fr 1.15fr 1fr 1fr') && css.includes('body[data-content-control-mode="delete"] .dcc-selectable *{pointer-events:none!important}') && css.includes('z-index:264') && css.includes('.dcc-selected'), 'delete mode must show four actions above page chrome and prevent nested item controls from opening while selected');
...
assert.ok(index.includes('content-controls.css?v=20260919-4') && index.includes('content-controls.js?v=20260919-4'), 'unified content control assets must load');
assert.ok(index.indexOf('content-controls.js?v=20260919-4') > index.indexOf('admin-console.js?v=20260918-4'), 'content controller must load after feature modules so it can centralize their actions');
