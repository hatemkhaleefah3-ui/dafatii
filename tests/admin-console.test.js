...
assert.ok(api.includes("teacherProfile ? validateTeacherProfileUpload(raw) : validateUpload(raw, context.env)") && api.includes('if (!teacherProfile) {') && api.includes('const driveUpload = !r2Upload && (teacherProfile || chatAttachment || usesDrive(context.env))'), 'teacher profile uploads must bypass Dafatii byte quota checks and force Google Drive storage');
