(() => {
  ...
  function openCourseSheet(course=null){
    ...
    root.innerHTML=`<div class="entity-sheet-overlay suite-overlay" id="course-overlay"><section class="entity-sheet suite-sheet course-sheet" role="dialog" aria-modal="true" aria-label="${course?'Edit course':'Add course'}"><div class="entity-sheet-handle"></div><div class="entity-sheet-head"><div><div class="eyebrow">Course workspace</div><h2>${course?'Edit course':'Add a course'}</h2></div><button class="icon-btn" id="course-close" aria-label="Close">×</button></div><form id="course-form"><div class="field"><label>Course name</label><input id="course-name" maxlength="80" required value="${esc(course?.name||'')}" placeholder="e.g. Computer Science — Year 2"></div>${course?'':`<div class="field"><label>Content template</label><select id="course-template">${templates.map(name=>`<option>${esc(name)}</option>`).join('')}</select><small class="course-field-help">Starts the course with tailored subjects, lectures, notes, resources, assignments, calendar items, rooms and conversations.</small></div>`}<div class="suite-form-grid"><div class="field"><label>Institution <span class="field-optional">Optional</span></label><input id="course-institution" maxlength="100" value="${esc(course?.institution||'')}" placeholder="School or university"></div><div class="field"><label>Term <span class="field-optional">Optional</span></label><input id="course-term" maxlength="80" value="${esc(course?.term||'')}" placeholder="e.g. Fall 2026"></div></div><button class="btn btn-primary auth-submit" type="submit">${course?'Save course':'Create and open course'}</button></form></section></div>`;
