(() => {
  ...
  function coursePage(){
    ...
    const canCreate=actor?.platformRole==='admin'||(actor?.accountType==='student'&&actor?.studentStage==='university');
  ...
  function courseCard(course){
    ...
    return `<article class="course-card ${active?'active':''}"><div class="course-card-top"><span>◇</span><b>${esc(statusLabel(status))}</b></div><h2>${esc(course.name)}</h2><p>${esc([course.learningField,course.difficultyLevel,course.institution,course.stage,money(course),course.visibility,course.joinPolicy==='approval'?'Approval required':'Direct join'].filter(Boolean).join(' · '))}</p><div class="course-card-actions">${status==='active'?`<button class="btn ${active?'btn-ghost':'btn-primary'}" data-course-switch="${esc(course.id)}" ${active?'disabled':''}>${active?'Currently open':'Open course'}</button>`:status?`<span class="course-status">${esc(statusLabel(status))}</span>`:`<button class="btn btn-primary" data-course-enroll="${esc(course.enrollmentCode)}" data-private="${course.visibility==='private'?'1':'0'}">Enroll</button>`}</div></article>`;
  }
  ...
  function openCourseSheet(){
    const templates=window.DafatiiCourses.templates(),actor=window.DafatiiCourses.actor||window.DafatiiAuth.user,isAdmin=actor?.platformRole==='admin';
    const adminMetadata=isAdmin?`<div class="field"><label>Learning field</label><input name="learningField" maxlength="80" placeholder="e.g. Programming, Languages, Design"></div><div class="field"><label>Difficulty</label><select name="difficultyLevel"><option value="beginner">Beginner</option><option value="intermediate">Intermediate</option><option value="advanced">Advanced</option><option value="expert">Expert</option></select></div>`:'';
    const visibility=isAdmin?`<div class="field"><label>Visibility</label><select name="visibility"><option value="public">Public</option><option value="private">Private by code</option></select></div>`:`<input type="hidden" name="visibility" value="private"><div class="field"><label>Visibility</label><div class="course-private-lock">Private · academic level, stage and field are locked to your signup profile</div></div>`;
    const close=sheet('Create a course',`<form id="course-form"><div class="field"><label>Course name</label><input name="name" maxlength="120" required></div><div class="field"><label>Content template</label><select name="templateName">${templates.map(name=>`<option>${esc(name)}</option>`).join('')}</select></div><div class="suite-form-grid"><div class="field"><label>Institution</label><input name="institution" maxlength="160"></div>${isAdmin?`<div class="field"><label>Stage</label><select name="stage"><option value="university" selected>Higher education</option><option value="independent">Independent</option></select></div>`:'<input type="hidden" name="stage" value="university">'}<div class="field"><label>Pricing</label><select name="pricing"><option value="free">Free</option><option value="paid">Paid</option></select></div><div class="field"><label>Price (minor units)</label><input name="priceMinor" type="number" min="0" value="0"></div>${visibility}<div class="field"><label>Join policy</label><select name="joinPolicy"><option value="approval">Needs acceptance</option><option value="direct">Direct join</option></select></div>${adminMetadata}</div><div class="field"><label>Private access code ${isAdmin?'<span class="field-optional">Required when private</span>':''}</label><input name="accessCode" type="password" minlength="6" maxlength="64" ${isAdmin?'':'required'}></div><button class="btn btn-primary auth-submit" type="submit">Create course</button><p class="auth-note" id="course-status"></p></form>`);
