(() => {
  ...
  function dafaaPage(){
    ...
    return `<section class="suite-page dafaa-manager">
      ...
      ${active.id?`<article class="dafaa-active-hero"><span class="dafaa-active-icon">◇</span><div><div class="eyebrow">Active dafaa · ${esc(active.membership?.role||'student')}</div><h2>${esc(active.name)}</h2><p>${esc([active.institution,active.stage].filter(Boolean).join(' · '))}</p></div><div class="dafaa-active-count"><strong>${state.subjects.length}</strong><span>subjects</span></div></article>`:'<article class="dafaa-explainer"><span>＋</span><div><h2>No active dafaa yet</h2><p>Join a dafaa, or create one from a representer account.</p></div></article>'}
  ...
  workspaceContent=function(page,parts,title){if(page==='change-dafaa')return dafaaPage();return previousWorkspaceContent(page,parts,title);};
  const previousWorkspace=workspace;
  workspace=function(current){previousWorkspace(current);enhanceDafaaSwitcher();if(current.split('/')[0]==='change-dafaa')bindDafaaManager();};
