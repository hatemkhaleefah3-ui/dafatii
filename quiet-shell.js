(() => {
  ...
  const paths = {
    ...
    'dafati':'M3 7 12 3l9 4-9 4zM6 10v7l6 4 6-4v-7', profile:'M12 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8M4 22v-2a8 8 0 0 1 16 0v2',
  ...
  const copy = {
    en:{workspace:'WORKSPACE',account:'Account & preferences',openMenu:'Open sidebar',closeMenu:'Close sidebar',skip:'Skip to content',dafat:'Dafat',primary:'Main navigation',accountNav:'Account navigation',preferences:'Preferences',opportunities:'Opportunities',appearance:'Appearance',language:'Language',light:'Light mode',dark:'Dark mode',english:'English',arabic:'Arabic',signout:'Sign out',signoutError:'Could not sign out. Please try again.',yourAccount:'Your account',dashboard:'Dashboard',subjects:'Library',calendar:'Calendar',chat:'Messages','study-rooms':'Study rooms',dafati:'Dafati',profile:'Profile',settings:'Settings',admin:'Administration',representer:'Dafaa management','apply-work':'Apply for work','apply-scholarship':'Scholarships',volunteer:'Volunteer','donate-us':'Support Dafatii',activeDafaa:'Active Dafaa',recentDafat:'Recent Dafat',seeAllDafat:'See all Dafat',seeProfile:'See full profile',student:'Student',owner:'Dafaa owner',email:'Email',stage:'Study stage',switching:'Opening…',back:'Go back'},
    ar:{workspace:'مساحة العمل',account:'الحساب والتفضيلات',openMenu:'فتح القائمة الجانبية',closeMenu:'إغلاق القائمة الجانبية',skip:'تخطي إلى المحتوى',dafat:'الدفعات',primary:'التنقل الرئيسي',accountNav:'تنقل الحساب',preferences:'التفضيلات',opportunities:'الفرص',appearance:'المظهر',language:'اللغة',light:'الوضع الفاتح',dark:'الوضع الداكن',english:'الإنجليزية',arabic:'العربية',signout:'تسجيل الخروج',signoutError:'تعذر تسجيل الخروج. حاول مرة أخرى.',yourAccount:'حسابك',dashboard:'لوحة التحكم',subjects:'المكتبة',calendar:'التقويم',chat:'الرسائل','study-rooms':'غرف الدراسة',dafati:'دفعتي',profile:'الملف الشخصي',settings:'الإعدادات',admin:'الإدارة',representer:'إدارة الدفعة','apply-work':'التقديم للعمل','apply-scholarship':'المنح الدراسية',volunteer:'التطوع','donate-us':'دعم دفاتري',activeDafaa:'الدفعة النشطة',recentDafat:'أحدث الدفعات',seeAllDafat:'عرض جميع الدفعات',seeProfile:'عرض الملف الشخصي كاملاً',student:'طالب',owner:'مالك الدفعة',email:'البريد الإلكتروني',stage:'المرحلة الدراسية',switching:'جارٍ الفتح…',back:'رجوع'}
  };
  ...
  const bottomIcons = Object.freeze({...mainNavIcons,dafati:'dafati',profile:'profile',settings:'settings'});
  ...
  function closePopovers(){
    ...
    document.querySelectorAll('[data-quiet-dafat],[data-quiet-profile]').forEach(button=>button.setAttribute('aria-expanded','false'));
  }
  function openDafaaPopover(trigger){
    if(document.querySelector('.quiet-dafaa-popover')){closePopovers();return;}
    closePopovers();trigger.setAttribute('aria-expanded','true');
    const active=window.DafatiiDafat.active();
    const enrolled=window.DafatiiDafat.list().filter(dafaa=>dafaa.membership?.status==='active');
    const recent=[active,...enrolled.filter(dafaa=>dafaa.id!==active.id).sort((a,b)=>Number(b.updatedAt||b.createdAt||0)-Number(a.updatedAt||a.createdAt||0))].filter(dafaa=>dafaa?.id).slice(0,3);
    const panel=document.createElement('section');panel.className='quiet-popover quiet-dafaa-popover';panel.setAttribute('aria-label',text('recentDafat'));
    panel.innerHTML=`<div class="quiet-popover-head"><strong>${escapeHtml(text('recentDafat'))}</strong><small>${escapeHtml(text('activeDafaa'))}</small></div><div class="quiet-dafaa-list">${recent.map(dafaa=>`<button data-switch-dafaa="${escapeHtml(dafaa.id)}" class="${dafaa.id===active.id?'active':''}"><span style="--dafaa-color:${escapeHtml(dafaa.color||'#1877f2')}">${escapeHtml(dafaa.icon||'◇')}</span><span><strong>${escapeHtml(dafaa.name)}</strong><small>${dafaa.id===active.id?escapeHtml(text('activeDafaa')):escapeHtml(dafaa.institution||'')}</small></span>${dafaa.id===active.id?`<b>${glyph('check')}</b>`:''}</button>`).join('')}</div><a class="quiet-popover-more" href="#dafati">${escapeHtml(text('seeAllDafat'))} ${glyph(language()==='ar'?'arrow-left':'arrow-right')}</a><p role="status"></p>`;
    trigger.parentElement.append(panel);
    panel.querySelector('.quiet-popover-more').onclick=closePopovers;
    panel.querySelectorAll('[data-switch-dafaa]').forEach(button=>button.onclick=async()=>{if(button.dataset.switchDafaa===active.id)return;panel.querySelector('[role=status]').textContent=text('switching');panel.querySelectorAll('button').forEach(item=>item.disabled=true);try{await window.DafatiiDafat.switchDafaa(button.dataset.switchDafaa);closePopovers();}catch(error){panel.querySelector('[role=status]').textContent=error.message;panel.querySelectorAll('button').forEach(item=>item.disabled=false);}});
  }
  function openProfilePopover(trigger){
    if(document.querySelector('.quiet-profile-popover')){closePopovers();return;}
    closePopovers();trigger.setAttribute('aria-expanded','true');
    const user=window.DafatiiAuth.user||{},active=window.DafatiiDafat.active();
  ...
  function enhance(){
    const shell=document.querySelector('.workspace,.pre-dafaa-shell');
    ...
    const primary=full?['dashboard','subjects','calendar','study-rooms','chat']:['dashboard','dafati','profile','settings'];
    ...
    const account=full?['dafati','profile','settings']:[];
    ...
    shell.querySelector('[data-quiet-dafat]').onclick=event=>{event.stopPropagation();openDafaaPopover(event.currentTarget);};
