(() => {
  ...
  const paths = {
    ...
    settings:'M4 6h16M4 12h16M4 18h16M9 3v6M15 9v6M9 15v6', 'school-teachers':'M4 4h7v16H4zM14 4h6v16h-6z', admin:'M12 3l8 4v5c0 5-3.4 8.8-8 10-4.6-1.2-8-5-8-10V7zM9 12l2 2 4-4',
  ...
  function sidebarContextNav(currentRoute){
    const parts=String(currentRoute||'').split('/'),page=parts[0];
    if(page==='subjects'&&parts[1]==='subject'&&parts[2]&&typeof SUBJECT_TABS!=='undefined'){
      const subjectId=parts[2],selected=String(parts[3]||'lectures').toLowerCase();
      return `<div class="quiet-sidebar-context"><small>${escapeHtml(language()==='ar'?'داخل المادة':'IN THIS SUBJECT')}</small>${SUBJECT_TABS.map(tab=>{const slug=String(tab).toLowerCase(),active=slug===selected;return `<a class="quiet-context-link ${active?'selected':''}" href="#subjects/subject/${subjectId}/${slug}" ${active?'aria-current="page"':''}>${escapeHtml(tab)}</a>`;}).join('')}</div>`;
    }
    const options=typeof MAIN_NAV!=='undefined'?MAIN_NAV[page]:null;
    if(!Array.isArray(options)||!options.length)return'';
    let selected='';try{selected=decodeURIComponent(parts.slice(1).join('/'));}catch{selected=parts.slice(1).join('/');}
    return `<div class="quiet-sidebar-context"><small>${escapeHtml(language()==='ar'?'داخل القسم':'IN THIS SECTION')}</small>${options.map((label,index)=>{const active=(!selected&&index===0)||selected===label;return `<a class="quiet-context-link ${active?'selected':''}" href="#${page}/${encodeURIComponent(label)}" ${active?'aria-current="page"':''}>${escapeHtml(label)}</a>`;}).join('')}</div>`;
  }
  ...
  function openCoursePopover(trigger){
    ...
    const schoolStudent=window.DafatiiAuth.user?.accountType==='student'&&window.DafatiiAuth.user?.studentStage==='school';
    const enrolled=window.DafatiiCourses.list().filter(course=>course.membership?.status==='active');
    const ordered=[active,...enrolled.filter(course=>course.id!==active.id).sort((a,b)=>Number(b.updatedAt||b.createdAt||0)-Number(a.updatedAt||a.createdAt||0))].filter(course=>course?.id);
    const choices=schoolStudent?ordered:ordered.slice(0,3);
    const panel=document.createElement('section');panel.className='quiet-popover quiet-course-popover';panel.setAttribute('aria-label',text(schoolStudent?'switchCourse':'recentCourses'));
    panel.innerHTML=`<div class="quiet-popover-head"><strong>${escapeHtml(text(schoolStudent?'switchCourse':'recentCourses'))}</strong><small>${escapeHtml(text('activeCourse'))}</small></div><div class="quiet-course-list">${choices.map(course=>`<button data-switch-course="${escapeHtml(course.id)}" class="${course.id===active.id?'active':''}"><span style="--course-color:${escapeHtml(course.color||'#1877f2')}">${escapeHtml(course.icon||'◇')}</span><span><strong>${escapeHtml(course.name)}</strong><small>${course.id===active.id?escapeHtml(text('activeCourse')):escapeHtml(course.institution||'')}</small></span>${course.id===active.id?`<b>${glyph('check')}</b>`:''}</button>`).join('')}</div><a class="quiet-popover-more" href="#change-course">${escapeHtml(text('seeAllCourses'))} ${glyph(language()==='ar'?'arrow-left':'arrow-right')}</a><p role="status"></p>`;
  ...
  function enhance(){
    ...
    const active=window.DafatiiCourses.active();
    const full=Boolean(active.id),currentRoute=route(),current=activePage(),user=window.DafatiiAuth.user,schoolStudent=user?.accountType==='student'&&user?.studentStage==='school';
    ...
    const account=full?[(user?.studentStage==='school'?'school-teachers':null),'change-course','profile','settings'].filter(Boolean):[];
    ...
    shell.querySelector('[data-quiet-menu]').onclick=()=>{
      if(window.matchMedia('(min-width:1200px)').matches){shell.classList.remove('quiet-sidebar-collapsed');return;}
