...
const PRE_DAFAA_ROUTES = new Set(['dashboard','dafati','profile','settings']);
const PRE_DAFAA_COPY = {
  en:{dashboard:'Dashboard',dafati:'Dafati',profile:'Profile',settings:'Settings',welcome:'Welcome to Dafatii',waiting:'Your account is ready. Create a Dafaa or enroll in one to open the full study workspace.',pending:'Pending applications',available:'Available Dafat',openDafat:'Open Dafati',appearance:'Appearance',language:'Language',light:'Light',dark:'Dark',english:'English',arabic:'Arabic',account:'Account details',type:'Account type',stage:'Student stage',check:'Check enrollment status'},
  ar:{dashboard:'لوحة التحكم',dafati:'دفعتي',profile:'الملف الشخصي',settings:'الإعدادات',welcome:'مرحباً بك في دفاتري',waiting:'حسابك جاهز. أنشئ دفعة أو سجّل في دفعة لفتح مساحة الدراسة الكاملة.',pending:'طلبات قيد الانتظار',available:'الدفعات المتاحة',openDafat:'فتح الدفعات',appearance:'المظهر',language:'اللغة',light:'فاتح',dark:'داكن',english:'الإنجليزية',arabic:'العربية',account:'تفاصيل الحساب',type:'نوع الحساب',stage:'المرحلة الدراسية',check:'التحقق من حالة التسجيل'}
};
const LANDING_COPY = {
...
const LABELS = {
  ...
  dafati: 'Dafati', 'change-language': 'Change Language', 'dark-mode': 'Change Dark Mode',
...
function join(){
  ...
  document.getElementById('auth-form').onsubmit = async e=>{
    ...
    try{
      ...
      state.joined = true; await window.DafatiiDafat.refresh(); setHash(window.DafatiiDafat.active().id?'dashboard/overview':'dafati');
    }catch(error){
...
function preDafaaContent(page,copy){
  ...
  if(page==='dafati')return workspaceContent('dafati',['dafati'],'Dafat');
  if(page==='profile'){
...
function workspace(current){
  ...
  <div class="app-shell workspace${sidebarClass}">
    ...
    <div class="settings-nav ${state.sidebar?'hidden':''}"><div class="settings-inner">
      ${settingAction('settings','Settings')}${settingAction('profile','Profile')}${settingAction('dafati','Dafati')}${settingAction('change-language','Change Language')}${settingAction('dark-mode','Change Dark Mode')}
    ...
    <aside class="sidebar ${state.sidebar?'open':''}">
      ...
      ${sideAction('settings','Settings')}${sideAction('profile','Profile')}${sideAction('dafati','Dafati')}${sideAction('change-language','Change Language')}${sideAction('dark-mode','Change Dark Mode')}
