...
async function enterAuthenticatedWorkspace(){
  state.joined=true;
  await window.DafatiiCourses.refresh();
  setHash(window.DafatiiAuth?.user?.studentStage==='school'?'dashboard':window.DafatiiCourses.active().id?'dashboard/overview':'change-course');
}
    ...
    <section class="auth-side">
      <div class="auth-card">
        ...
        <form id="auth-form">
          ...
          <div class="field"><label>${escapeHtml(isSignup?c.email:c.identifier)}</label><input type="${isSignup?'email':'text'}" name="${isSignup?'email':'identifier'}" autocomplete="${isSignup?'email':'username'}" inputmode="${isSignup?'email':'text'}" placeholder="${escapeHtml(isSignup?'you@example.com':c.identifierPlaceholder)}" required></div>
          <div class="field"><label>${escapeHtml(c.password)}</label><input type="password" name="password" minlength="${isSignup?12:4}" maxlength="256" autocomplete="${isSignup?'new-password':'current-password'}" placeholder="••••••••••••" required></div>
