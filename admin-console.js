(() => {
  ...
  function teacherCard(teacher){
    ...
    return `<article class="admin-teacher-card" data-admin-teacher="${esc(teacher.id)}">
      ...
      <div class="admin-teacher-actions"><button class="btn btn-ghost btn-small" data-teacher-edit="${esc(teacher.id)}">${esc(tx('Edit','تعديل'))}</button><button class="btn btn-danger btn-small" data-teacher-delete="${esc(teacher.id)}">${esc(tx('Delete','حذف'))}</button></div>
    </article>`;
  }
  ...
  async function bind(){
    ...
    document.querySelectorAll('[data-teacher-delete]').forEach(button=>button.onclick=async()=>{if(!confirm(tx('Remove this teacher from the directory?','إزالة هذا المدرس من الدليل؟')))return;try{await window.DafatiiApi.request(`/admin/teachers/${button.dataset.teacherDelete}`,{method:'DELETE'});data.loaded=false;await load(true);}catch(error){data.error=error.message;render();}});
