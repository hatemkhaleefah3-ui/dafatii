(() => {
  ...
  function coursesView(){
    return shell(`<div class="admin-console-section-head"><div><div class="eyebrow">${esc(tx('Higher education','التعليم العالي'))}</div><h2>${esc(tx('Courses','الدورات'))}</h2><p>${esc(tx('School students cannot create Courses, but can enroll after teacher setup. Higher-education students may create private Courses; only administrators publish public Courses.','طلاب المدارس لا يمكنهم إنشاء الدورات، لكن يمكنهم الانضمام بعد إعداد المدرسين. طلاب التعليم الأعلى يمكنهم إنشاء دورات خاصة، والمشرف فقط ينشر الدورات العامة.'))}</p></div><button class="btn btn-primary" id="admin-create-course">＋ ${esc(tx('Create course','إنشاء دورة'))}</button></div>
