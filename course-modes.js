(() => {
  ...
  function defaultLanguageContent(selectedLanguage=targetLanguage()){
    ...
    pages[loc+'language-examine']=[
      {id:'seed-examine-1',title:'Checkpoint',body:'Add a simple question or examination prompt here.'},
      {id:'seed-examine-2',title:'Review',body:'Use this item for another simple examination prompt.'}
    ];
  ...
  function renderVoiceItem(item,index){
    ...
    const action=speaking?'<button type="button" class="voice-action mic-action" data-language-record><span class="voice-action-icon" aria-hidden="true">●</span><span><strong>Start microphone</strong><small>Record and play back your pronunciation</small></span></button><div class="voice-record-status" data-language-record-status>Microphone starts only when you press the button.</div><audio class="voice-playback" data-language-record-playback controls hidden></audio>':'<button type="button" class="voice-action hear-action" data-language-speak="'+esc(text)+'"><span class="voice-action-icon" aria-hidden="true">▶</span><span><strong>Hear pronunciation</strong><small>Use your browser voice for '+esc(targetLanguage())+'</small></span></button><div class="voice-record-status" data-language-speech-status></div>';
    return '<article class="language-learning-card voice-card '+(speaking?'speaking-card':'hearing-card')+'" data-language-item="'+esc(item.id)+'"><div class="language-item-kicker"><span>'+esc(label)+'</span><b>'+String(index+1).padStart(2,'0')+'</b></div><div class="'+(sentence?'voice-sentence':'voice-word')+'">'+esc(text)+'</div><p class="voice-instruction">'+esc(item.instruction||'Practice carefully, then continue when you are ready.')+'</p>'+action+'</article>';
  }
  ...
  function languageContentPage(page){
    ...
    const content=readLanguageContent(),loc=currentLocation(),items=itemsFor(content,page,loc),learner=readLearnerState(),index=languagePageIndex(page,loc,items),item=items[index],markup=item?(page==='language-letters'?renderVocabularyItem(item,index):page==='language-voice'?renderVoiceItem(item,index):renderGenericLanguageItem(item,index)):'<article class="language-learning-card empty"><div class="language-item-kicker"><span>Content</span><b>00</b></div><h2>No content yet</h2><p class="language-item-prompt">Use Content Control to add a content item.</p></article>';
  ...
  function bindLanguagePage(page){
    ...
    if(LANGUAGE_ROUTES.includes(page)&&page!=='language-home'){
      ...
      if(page==='language-voice'){document.querySelector('.voice-card')?.setAttribute('data-item-type',items[index]?.type||'');document.querySelectorAll('[data-language-speak]').forEach(button=>button.addEventListener('click',()=>playLanguageSpeech(button.dataset.languageSpeak,button.closest('.voice-card')?.querySelector('[data-language-speech-status]'))));document.querySelectorAll('[data-language-record]').forEach(button=>button.addEventListener('click',()=>toggleLanguageRecording(button)));}
      return;
    }
  ...
  function languageDraftHasContent(page,item){if(page==='language-letters')return item.word!=='Word'||Boolean(item.sentence||item.meaningEnglish||item.meaningArabic||item.imageUrl);if(page==='language-voice')return item.text!=='Practice'||Boolean(item.instruction);if(page==='language-grammar')return Boolean(item.body||item.answer||!['Grammar / rule','Note','Example','Training'].includes(item.title));return item.title!=='Untitled'||Boolean(item.body||(item.choices||[]).length);}
  async function importLanguageFiles(files,page){
    const result=[];for(const file of [...files].slice(0,12)){const name=String(file.name||'Imported file'),lower=name.toLowerCase(),pushValues=values=>{if(!values.length)return;if(page==='language-letters')result.push({id:languageUid('import'),type:'vocabulary',word:values[0]||'Word',meaningEnglish:values.slice(1).join(' · '),meaningArabic:''});else if(page==='language-voice')result.push({id:languageUid('import'),type:'hearing-word',text:values[0]||'Practice',instruction:values.slice(1).join(' · ')});else if(page==='language-grammar')result.push({id:languageUid('import'),type:'grammar-note',title:values[0]||'Note',body:values.slice(1).join(' · '),answer:''});else result.push({id:languageUid('import'),title:values[0]||'Imported content',body:values.slice(1).join(' · ')});};
