(() => {
  ...
  function defaultLanguageContent(selectedLanguage=targetLanguage()){
    ...
    const pageItems={
      'language-letters':[
        {id:'seed-letters-1',title:'Starter vocabulary',body:'Add a useful word, phrase, or writing prompt here.'},
        {id:'seed-letters-2',title:'Writing practice',body:'Use this item for a short vocabulary or writing activity.'}
      ],
      'language-voice':[
        {id:'seed-voice-1',title:'Listening practice',body:'Add a listening or speaking instruction here.'},
        {id:'seed-voice-2',title:'Speaking practice',body:'Use this item for a simple speaking activity.'}
      ],
      'language-grammar':[
        {id:'seed-grammar-1',title:'Grammar note',body:'Add a grammar explanation, rule, or example here.'},
        {id:'seed-grammar-2',title:'Grammar practice',body:'Use this item for a simple grammar activity.'}
      ],
      'language-examine':[
    ...
    Object.entries(pageItems).forEach(([page,items])=>{pages[loc+page]=items;});
  ...
  function normalizeLanguageContent(value){
    ...
    content.version=2;
    content.pages=content.pages&&typeof content.pages==='object'?content.pages:{};
    content.video=content.video&&typeof content.video==='object'?content.video:{};
    Object.keys(content.pages).forEach(key=>{
      const page=key.split('|').pop();
      if(page==='language-video'){
        content.pages[key]=[];
        if(!content.video[key])content.video[key]={title:'Watch and understand',prompt:'Write what you understood from this video in your own words.',url:''};
        return;
      }
      const items=Array.isArray(content.pages[key])?content.pages[key]:[];
      content.pages[key]=items.map(item=>({id:String(item?.id||languageUid('content')),title:String(item?.title||'Untitled'),body:String(item?.body||'')}));
    });
    return content;
  }
  ...
  function languageContentPage(page){
    ...
    return '<section class="language-content-page language-item-process" data-language-content-page="'+esc(page)+'"><header class="language-page-head"><div><small>Level '+Math.max(1,Number(learner.currentLevel)||1)+' · Step '+Math.max(1,Number(learner.currentStep)||1)+' · Box '+Math.max(1,Number(learner.currentBox)||1)+'</small><h1>'+esc(routeTitle(page))+'</h1><p>'+esc(targetLanguage())+' · item '+(items.length?index+1:0)+' of '+items.length+'</p></div></header><div class="language-step-progress"><span style="width:'+Math.round(((index+1)/Math.max(items.length,1))*100)+'%"></span></div>'+(item?'<article class="language-learning-card" data-language-item="'+esc(item.id)+'"><div class="language-item-kicker"><span>Content</span><b>'+String(index+1).padStart(2,'0')+'</b></div><h2>'+esc(item.title||'Untitled')+'</h2><p class="language-item-prompt">'+esc(item.body||'')+'</p></article>':'<article class="language-learning-card empty"><div class="language-item-kicker"><span>Content</span><b>00</b></div><h2>No content yet</h2><p class="language-item-prompt">Use Content Control to add a content item.</p></article>')+'<footer class="language-item-navigation"><button type="button" class="btn btn-ghost" data-language-item-prev '+(index===0?'disabled':'')+'>Previous</button><span>'+(items.length?index+1:0)+' / '+items.length+'</span><button type="button" class="btn btn-primary" data-language-item-next '+(!items.length||index>=items.length-1?'disabled':'')+'>Next</button></footer></section>';
  }
  ...
  function readLanguageEditorData(form,page,base={}){
    const data=Object.fromEntries(new FormData(form)),next={...base,title:String(data.title||'').trim()||'Untitled',body:String(data.body||'').trim()};
    delete next.type;delete next.feature;delete next.answer;delete next.options;delete next.hint;delete next.mediaUrl;delete next.label;
    if(page==='language-level-test')next.choices=String(data.choices||'').split(/\n+/).map(value=>value.trim()).filter(Boolean);else delete next.choices;
    return next;
  }
  function renderLanguageImportStep(originPage,page,loc,target,close){
    ...
    step.onsubmit=async event=>{event.preventDefault();const status=step.querySelector('[data-language-import-status]'),button=step.querySelector('button[type=submit]');button.disabled=true;status.textContent='Adding…';try{const content=readLanguageContent();let destination={...loc};if(!LANGUAGE_INTERMEDIATE_ROUTES.includes(page)&&target!=='language'){if(target==='level'){const number=(content.levels?.length||0)+1,newLevel={id:'level-'+number,name:'Level '+number,steps:[{id:'step-1',name:'Step 1',boxes:[{id:'box-1',name:'Box 1'}]}]};content.levels.push(newLevel);destination={level:newLevel.id,step:'step-1',box:'box-1'};}else{const level=content.levels.find(item=>item.id===loc.level)||content.levels[0];if(target==='step'){const number=(level.steps?.length||0)+1,newStep={id:'step-'+number,name:'Step '+number,boxes:[{id:'box-1',name:'Box 1'}]};level.steps.push(newStep);destination={level:level.id,step:newStep.id,box:'box-1'};}if(target==='box'){const stepObj=(level.steps||[]).find(item=>item.id===loc.step)||level.steps?.[0],number=(stepObj.boxes?.length||0)+1,newBox={id:'box-'+number,name:'Box '+number};stepObj.boxes.push(newBox);destination={level:level.id,step:stepObj.id,box:newBox.id};}}}
      const draft=readLanguageEditorData(step,page,{id:languageUid('content')}),imported=await importLanguageFiles(step.querySelector('[data-language-import-files]').files||[]),hasDraft=Boolean(draft.title!=='Untitled'||draft.body||(draft.choices||[]).length),additions=[...imported];if(hasDraft||!imported.length)additions.unshift(draft);
