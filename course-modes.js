(() => {
  ...
  const LANGUAGE_PAGE_ITEM_TYPES=Object.freeze({
    ...
    'language-voice':[
      ...
      {id:'speaking-word',label:'Speaking · word'},
      {id:'hearing-sentence',label:'Hearing · sentence'},
      {id:'speaking-sentence',label:'Speaking · sentence'}
    ],
    'language-grammar':[
  ...
  function defaultLanguageContent(selectedLanguage=targetLanguage()){
    ...
    pages[loc+'language-letters']=[
      ...
      {id:'seed-vocab-3',type:'image-vocabulary',word:'Image word',imageUrl:'',imageAlt:'Image representing the word'}
    ];
    pages[loc+'language-voice']=[
      ...
      {id:'seed-voice-2',type:'speaking-word',text:'New word',instruction:'Record yourself saying the word, then play it back.'},
      {id:'seed-voice-3',type:'hearing-sentence',text:'This is a practice sentence.',instruction:'Listen to the whole sentence and notice its rhythm.'},
      {id:'seed-voice-4',type:'speaking-sentence',text:'This is a practice sentence.',instruction:'Record the sentence, then listen to your playback.'}
    ];
    ...
    return {version:3,targetLanguage:target,levels:[{id:'level-1',name:'Level 1',steps:[{id:'step-1',name:'Step 1',boxes:[{id:'box-1',name:'Box 1'}]}]}],pages,video:{[loc+'language-video']:{title:'Watch and understand',prompt:'Write what you understood from this video in your own words.',url:''}},intermediate:{
  ...
  function normalizeLanguageContent(value){
    ...
    content.version=3;content.pages=content.pages&&typeof content.pages==='object'?content.pages:{};content.video=content.video&&typeof content.video==='object'?content.video:{};
  ...
  function renderVocabularyItem(item,index){
    const type=pageItemType('language-letters',item.type)?.id||'vocabulary',meaning=nativeMeaning(item),native=nativeLanguage(),word=String(item.word||'Word');
    if(type==='image-vocabulary'){
      const image=item.imageUrl?'<img src="'+esc(item.imageUrl)+'" alt="'+esc(item.imageAlt||word)+'" loading="lazy" referrerpolicy="no-referrer">':'<div class="vocab-image-empty"><span>'+esc(word.slice(0,1).toUpperCase())+'</span><small>Add an image URL from Content Control</small></div>';
      return '<article class="language-learning-card vocab-card image-vocab-card" data-language-item="'+esc(item.id)+'"><div class="language-item-kicker"><span>Image vocabulary</span><b>'+String(index+1).padStart(2,'0')+'</b></div><figure class="vocab-image-frame">'+image+'</figure><div class="vocab-word-row"><h2>'+esc(word)+'</h2><span>See it · remember it</span></div></article>';
    }
    if(type==='sentence-vocabulary')return '<article class="language-learning-card vocab-card sentence-vocab-card" data-language-item="'+esc(item.id)+'"><div class="language-item-kicker"><span>Sentence vocabulary</span><b>'+String(index+1).padStart(2,'0')+'</b></div><div class="vocab-word-chip">'+esc(word)+'</div><blockquote>'+esc(item.sentence||'Add a sentence for this word.')+'</blockquote><div class="vocab-meaning-panel" dir="'+(native==='Arabic'?'rtl':'ltr')+'"><small>'+esc(native)+' meaning</small><strong>'+esc(meaning||'Meaning not added yet')+'</strong></div></article>';
    return '<article class="language-learning-card vocab-card" data-language-item="'+esc(item.id)+'"><div class="language-item-kicker"><span>Vocabulary</span><b>'+String(index+1).padStart(2,'0')+'</b></div><div class="vocab-main-word">'+esc(word)+'</div><div class="vocab-meaning-panel" dir="'+(native==='Arabic'?'rtl':'ltr')+'"><small>'+esc(native)+' meaning</small><strong>'+esc(meaning||'Meaning not added yet')+'</strong></div><div class="vocab-native-badge">'+(native==='Arabic'?'العربية':'English')+'</div></article>';
  }
  function renderVoiceItem(item,index){
    const type=pageItemType('language-voice',item.type)?.id||'hearing-word',speaking=type.startsWith('speaking-'),sentence=type.endsWith('-sentence'),label=(speaking?'Speaking':'Hearing')+' · '+(sentence?'sentence':'word'),text=String(item.text||'Practice');
    const hear='<button type="button" class="voice-action hear-action" data-language-speak="'+esc(text)+'"><span class="voice-action-icon" aria-hidden="true">▶</span><span><strong>Hear pronunciation</strong><small>Listen to '+(sentence?'the sentence':'the word')+' in '+esc(targetLanguage())+'</small></span></button>';
    const mic='<button type="button" class="voice-action mic-action" data-language-record><span class="voice-action-icon mic-icon" aria-hidden="true">●</span><span><strong>Open microphone</strong><small>Record yourself and play it back</small></span></button>';
    return '<article class="language-learning-card voice-card '+(speaking?'speaking-card':'hearing-card')+'" data-language-item="'+esc(item.id)+'" data-item-type="'+esc(type)+'"><div class="language-item-kicker"><span>'+esc(label)+'</span><b>'+String(index+1).padStart(2,'0')+'</b></div><div class="'+(sentence?'voice-sentence':'voice-word')+'">'+esc(text)+'</div><p class="voice-instruction">'+esc(item.instruction||'Practice carefully, then continue when you are ready.')+'</p><div class="voice-practice-actions">'+(speaking?mic+hear:hear+mic)+'</div><div class="voice-record-status" data-language-speech-status></div><div class="voice-record-status" data-language-record-status>Microphone opens only after you press Open microphone.</div><audio class="voice-playback" data-language-record-playback controls hidden></audio></article>';
  }
  function renderExamItem(item,index){
    ...
    if(type==='exam-multiple-choice')return '<article class="language-learning-card exam-card exam-multiple-card" data-language-item="'+esc(item.id)+'"><div class="language-item-kicker"><span>Multiple choice</span><b>'+number+'</b></div><h2>'+question+'</h2><div class="exam-multiple-grid">'+choices.map(choice=>'<label><input type="checkbox" value="'+esc(choice)+'" data-exam-multiple-option><span>'+esc(choice)+'</span></label>').join('')+'</div><button type="button" class="exam-submit" data-exam-multiple-check="'+esc((item.answers||[]).join('|'))+'">Check selected answers</button><div class="exam-feedback" aria-live="polite"></div></article>';
    if(type==='exam-true-false')return '<article class="language-learning-card exam-card exam-truefalse-card" data-language-item="'+esc(item.id)+'"><div class="language-item-kicker"><span>True / false</span><b>'+number+'</b></div><div class="exam-statement-mark">?</div><h2>'+question+'</h2><div class="exam-truefalse-actions"><button type="button" data-exam-choice="True" data-exam-answer="'+esc(item.answer||'True')+'">True</button><button type="button" data-exam-choice="False" data-exam-answer="'+esc(item.answer||'True')+'">False</button></div><div class="exam-feedback" aria-live="polite"></div></article>';
    if(type==='exam-fill-blank')return '<article class="language-learning-card exam-card exam-fill-card" data-language-item="'+esc(item.id)+'"><div class="language-item-kicker"><span>Fill in the blank</span><b>'+number+'</b></div><div class="exam-fill-line"></div><h2>'+question+'</h2><div class="exam-text-answer"><input type="text" data-exam-text-input placeholder="Type the missing answer…"><button type="button" data-exam-text-check="'+esc(item.answer||'')+'">Check</button></div><div class="exam-feedback" aria-live="polite"></div></article>';
    if(type==='exam-short-answer')return '<article class="language-learning-card exam-card exam-short-card" data-language-item="'+esc(item.id)+'"><div class="language-item-kicker"><span>Short answer</span><b>'+number+'</b></div><h2>'+question+'</h2><textarea rows="6" class="exam-short-textarea" data-exam-short-input placeholder="Write your answer in your own words…"></textarea><div class="exam-short-actions"><button type="button" data-exam-short-review="'+esc(item.answer||'')+'">Review answer</button></div><div class="exam-model-answer" hidden><small>Model answer</small><strong>'+esc(item.answer||'No model answer has been added.')+'</strong></div><div class="exam-feedback" aria-live="polite"></div></article>';
    return '<article class="language-learning-card exam-card exam-single-card" data-language-item="'+esc(item.id)+'"><div class="language-item-kicker"><span>Single choice</span><b>'+number+'</b></div><h2>'+question+'</h2><div class="exam-choice-stack">'+choices.map((choice,choiceIndex)=>'<button type="button" data-exam-choice="'+esc(choice)+'" data-exam-answer="'+esc(item.answer||'')+'"><span>'+String.fromCharCode(65+choiceIndex)+'</span><strong>'+esc(choice)+'</strong></button>').join('')+'</div><div class="exam-feedback" aria-live="polite"></div></article>';
  }
  function renderGenericLanguageItem(item,index){return '<article class="language-learning-card generic-language-card" data-language-item="'+esc(item.id)+'"><div class="language-item-kicker"><span>Content</span><b>'+String(index+1).padStart(2,'0')+'</b></div><h2>'+esc(item.title||'Untitled')+'</h2><p class="language-item-prompt">'+esc(item.body||'')+'</p></article>';}
  function grammarItemMarkup(item,index){
    ...
    if(type==='grammar-law')return '<article class="grammar-law-card" id="language-item-'+esc(item.id)+'" data-language-item="'+esc(item.id)+'"><div class="grammar-law-badge">LAW</div><small>Grammar / rule · '+number+'</small><h2>'+esc(item.title||'Grammar / rule')+'</h2><p>'+esc(item.body||'')+'</p></article>';
    if(type==='grammar-example')return '<article class="grammar-thread-card grammar-example-card" id="language-item-'+esc(item.id)+'" data-language-item="'+esc(item.id)+'"><div class="grammar-thread-icon">EX</div><div><small>Example · '+number+'</small><h3>'+esc(item.title||'Example')+'</h3><p>'+esc(item.body||'')+'</p></div></article>';
    if(type==='grammar-training')return '<article class="grammar-thread-card grammar-training-card" id="language-item-'+esc(item.id)+'" data-language-item="'+esc(item.id)+'"><div class="grammar-thread-icon">TR</div><div><small>Training · '+number+'</small><h3>'+esc(item.title||'Training')+'</h3><p>'+esc(item.body||'')+'</p><div class="grammar-training-response"><input type="text" data-grammar-training-input placeholder="Write your answer…"><button type="button" data-grammar-training-check="'+esc(item.answer||'')+'">Check</button></div><div class="grammar-training-feedback" aria-live="polite"></div></div></article>';
    return '<article class="grammar-thread-card grammar-note-card" id="language-item-'+esc(item.id)+'" data-language-item="'+esc(item.id)+'"><div class="grammar-thread-icon">NT</div><div><small>Note · '+number+'</small><h3>'+esc(item.title||'Note')+'</h3><p>'+esc(item.body||'')+'</p></div></article>';
  }
  ...
  function languageVideoPage(){
    ...
    return '<section class="language-content-page language-video-page" data-language-content-page="language-video"><header class="language-page-head"><div><small>Level '+Math.max(1,Number(learner.currentLevel)||1)+' · Step '+Math.max(1,Number(learner.currentStep)||1)+' · Box '+Math.max(1,Number(learner.currentBox)||1)+'</small><h1>'+esc(config.title||routeTitle('language-video'))+'</h1><p>'+esc(targetLanguage())+' · watch, understand, explain</p></div></header><div class="language-video-player">'+player+'</div><article class="language-video-notes"><div class="video-note-intro"><div class="video-note-icon">✎</div><div><small>Your understanding</small><h2>Capture what you learned</h2><p>'+esc(config.prompt||'Write the main ideas in your own words.')+'</p></div></div><div class="video-note-editor"><div class="video-note-editor-head"><span>Learning note</span><small><b data-language-video-word-count>0</b> words</small></div><textarea rows="10" data-language-video-note placeholder="Write the key ideas, vocabulary, examples, or questions you understood from the video…">'+esc(note)+'</textarea><div class="video-note-editor-foot"><span data-language-video-note-status>Private to this browser</span><button type="button" data-language-video-note-save><span>Save note</span><b>⌘S</b></button></div></div></article></section>';
  }
  ...
  let activeLanguageRecorder=null,activeLanguageStream=null,activeLanguageRecordingUrl='';
  ...
  async function toggleLanguageRecording(button){
    ...
    stopLanguageRecorder();if(activeLanguageRecordingUrl){URL.revokeObjectURL(activeLanguageRecordingUrl);activeLanguageRecordingUrl='';}
    try{
  ...
  function bindLanguagePage(page){
    ...
    if(LANGUAGE_ROUTES.includes(page)&&page!=='language-home'){
      ...
      document.querySelector('[data-language-item-prev]')?.addEventListener('click',()=>{stopLanguageRecorder();setLanguagePageIndex(page,loc,index-1);render();});
      document.querySelector('[data-language-item-next]')?.addEventListener('click',()=>{stopLanguageRecorder();setLanguagePageIndex(page,loc,index+1);render();});
      if(page==='language-voice'){document.querySelectorAll('[data-language-speak]').forEach(button=>button.addEventListener('click',()=>playLanguageSpeech(button.dataset.languageSpeak,button.closest('.voice-card')?.querySelector('[data-language-speech-status]'))));document.querySelectorAll('[data-language-record]').forEach(button=>button.addEventListener('click',()=>toggleLanguageRecording(button)));}
      if(page==='language-examine'){
  ...
  function syncLanguageEditorType(form){
    const select=form.querySelector('[data-language-editor-type]');if(!select)return;const type=select.value;form.querySelectorAll('[data-language-types]').forEach(node=>{const allowed=String(node.dataset.languageTypes||'').split(',');node.hidden=!allowed.includes(type);});select.onchange=()=>syncLanguageEditorType(form);
  }
