(() => {
  ...
  function bindLetterDrawing(){
    ...
    let upperResult={valid:false,score:0},lowerResult={valid:false,score:0},pronunciationPassed=!gate;
    const update=()=>{
      if(alreadyDone){
      ...
      if(feedback)feedback.textContent=valid
        ? '✓ Shapes and microphone pronunciation are ready. Complete '+letter+' to continue.'
        : 'Finish all checks · uppercase '+upperPct+'% · lowercase '+lowerPct+'% · pronunciation '+(pronunciationPassed?'✓':'needed')+'.';
    };
    if(gate){
      const mic=document.querySelector('[data-letter-pronunciation-record]'),field=document.querySelector('[data-letter-pronunciation-transcript]'),pronunciationFeedback=document.querySelector('[data-letter-pronunciation-feedback]');
      mic?.addEventListener('click',()=>{
        const Recognition=window.SpeechRecognition||window.webkitSpeechRecognition;
        if(!Recognition){if(pronunciationFeedback)pronunciationFeedback.textContent='Microphone speech recognition is not available in this browser.';return;}
        ...
        if(pronunciationFeedback)pronunciationFeedback.textContent='Listening… say '+letter+'.';
        recognition.onresult=event=>{
          const transcript=String(event.results[0][0].transcript||'').trim(),spoken=normalizeText(transcript),name=normalizeText(LETTER_SPEECH[letter]||letter),symbol=normalizeText(letter);
          if(field)field.value=transcript;
          pronunciationPassed=spoken===symbol||spoken===name||spoken==='letter '+symbol;
          if(pronunciationFeedback)pronunciationFeedback.textContent=pronunciationPassed?'✓ Letter pronunciation recognized.':'Not recognized as '+letter+'. Try again.';
          update();
        };
        recognition.onerror=()=>{pronunciationPassed=false;if(pronunciationFeedback)pronunciationFeedback.textContent='Microphone recognition failed. Try again.';update();};
        recognition.start();
      });
    }
