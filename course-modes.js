(() => {
  ...
  const COURSE_TYPES = ['dafaa','personal','teaching'];
  ...
  function courseType(){ const type=String(courseMeta().courseType || 'dafaa'); return COURSE_TYPES.includes(type)?type:'dafaa'; }
  function wrapCourseCreation(){
    ...
    api.createCourse=async input => {
      ...
      const course=await originalCreate({...input,courseType:type});
      const suite=readSuite();
      suite.courseMeta={version:1,courseType:type,studyType:input.studyType||'courses'};
      writeSuite(suite);
  ...
  function adaptNavigation(){
    const type=courseType(),current=(location.hash||'#dashboard/overview').replace(/^#\/?/,'').split('/')[0];
  ...
  function installCourseChangeRouting(){
    window.addEventListener('dafatii:coursechanged',()=>{
      ...
      setTimeout(()=>setHash('dashboard/overview'),0);
    });
  }
