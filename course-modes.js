(() => {
  ...
  function languageHomePage(){
    const learner=readLearnerState(),target=targetLanguage(),student=window.DafatiiCourses.active().membership?.role==='student',unfinished=student&&learner.entryMode&&!learner.onboardingComplete,continueRoute=unfinished?(learner.entryMode==='zero'?'language-start-zero':'language-level-test'):'language-letters',pathLabel=learner.entryMode==='zero'?'Start from zero':learner.entryMode==='test'?'Level check':'Course workspace',level=Math.max(1,Number(learner.currentLevel)||1),step=Math.max(1,Number(learner.currentStep)||1),box=Math.max(1,Number(learner.currentBox)||1);
    return '<section class="language-home-page"><header class="language-home-hero"><div class="language-home-copy"><small>'+esc(target)+' · structured language course</small><h1>'+esc(target)+'</h1><p>Move from recognition to recall, then production. Each session keeps one clear objective and immediate feedback.</p><div class="language-home-signals" aria-label="Core language skills"><span>Vocabulary</span><span>Listening</span><span>Speaking</span><span>Grammar</span></div><button type="button" data-language-continue="'+esc(continueRoute)+'">'+(unfinished?'Continue setup':'Continue learning')+' <span aria-hidden="true">›</span></button></div><aside class="language-home-emblem" aria-label="Current learning position"><small>Current position</small><strong>Level '+level+'</strong><span>Step '+step+' · Box '+box+'</span><em>Listen · Speak · Read · Write</em></aside></header><div class="language-home-grid"><article><small>Learning loop</small><strong>Recognize → Recall → Produce</strong><span>Practice is sequenced for retrieval, not passive browsing.</span></article><article><small>Support language</small><strong>'+esc(learner.nativeLanguage||'Not selected')+'</strong><span>Used for meaning, comparison, and instructions where needed.</span></article><article><small>Course path</small><strong>'+esc(pathLabel)+'</strong><span>Level '+level+' · Step '+step+' · Box '+box+'</span></article></div></section>';
  }
  ...
  function languageGrammarPage(){
    const content=readLanguageContent(),loc=currentLocation(),items=itemsFor(content,'language-grammar',loc),learner=readLearnerState(),laws=items.filter(item=>item.type==='grammar-law'),support=items.filter(item=>item.type!=='grammar-law'),level=Math.max(1,Number(learner.currentLevel)||1),step=Math.max(1,Number(learner.currentStep)||1),box=Math.max(1,Number(learner.currentBox)||1);
    return '<section class="language-content-page grammar-lesson-page" data-language-content-page="language-grammar"><header class="language-page-head"><div class="language-page-copy"><small>'+esc(targetLanguage())+' · guided grammar</small><h1>'+esc(routeTitle('language-grammar'))+'</h1><p>Understand the rule, inspect the evidence, then apply it without looking back.</p></div><div class="language-session-position" aria-label="Current learning position"><span><small>Level</small><strong>'+level+'</strong></span><span><small>Step</small><strong>'+step+'</strong></span><span><small>Box</small><strong>'+box+'</strong></span></div></header><div class="language-lesson-brief"><small>Learning sequence</small><strong>Rule <span>→</span> Notes <span>→</span> Examples <span>→</span> Practice</strong></div><div class="grammar-lesson-shell"><section class="grammar-law-stack">'+(laws.length?laws.map((item,index)=>grammarItemMarkup(item,index)).join(''):'<article class="grammar-law-card empty"><div class="grammar-law-badge">LAW</div><small>Grammar / rule</small><h2>No rule yet</h2><p>Add the grammar or rule from Content Control.</p></article>')+'</section><div class="grammar-connection"><span></span><strong>Understand the rule first, then use the supporting content to test it.</strong><span></span></div><section class="grammar-thread">'+(support.length?support.map((item,index)=>grammarItemMarkup(item,laws.length+index)).join(''):'<article class="grammar-thread-card grammar-note-card empty"><div class="grammar-thread-icon">+</div><div><small>Lesson content</small><h3>No supporting content yet</h3><p>Add notes, examples, and training items.</p></div></article>')+'</section></div></section>';
  }
  ...
  function languageVideoPage(){
    const content=readLanguageContent(),loc=currentLocation(),learner=readLearnerState(),config=languageVideoConfig(content,loc),index=languagePageIndex('language-video',loc,[0,1]),stage=index===1?languageStoryReadingPage(config,loc,index):languageVideoUnderstandingPage(config,loc,index),progress=(index+1)*50,level=Math.max(1,Number(learner.currentLevel)||1),step=Math.max(1,Number(learner.currentStep)||1),box=Math.max(1,Number(learner.currentBox)||1);
    return '<section class="language-content-page language-video-page watching-reading-page" data-language-content-page="language-video"><header class="language-page-head watching-reading-head"><div class="language-page-copy"><small>'+esc(targetLanguage())+' · comprehension session</small><h1>'+esc(routeTitle('language-video'))+'</h1><p>Watch for meaning, read for detail, then explain what you understood in your own words.</p></div><div class="language-session-position" aria-label="Current learning position"><span><small>Level</small><strong>'+level+'</strong></span><span><small>Step</small><strong>'+step+'</strong></span><span><small>Box</small><strong>'+box+'</strong></span></div></header><div class="language-progress-block"><div class="language-step-progress watch-read-progress" role="progressbar" aria-label="Comprehension session progress" aria-valuemin="0" aria-valuemax="100" aria-valuenow="'+progress+'"><span style="width:'+progress+'%"></span></div><div class="language-progress-meta"><span>Part '+(index+1)+' of 2</span><strong>'+progress+'%</strong></div></div>'+stage+'<footer class="language-item-navigation watch-read-navigation"><button type="button" class="btn btn-ghost" data-watch-read-prev aria-label="Previous comprehension part" '+(index===0?'disabled':'')+'>Previous</button><span><strong>'+(index+1)+'</strong><small>of 2</small></span><button type="button" class="btn btn-primary" data-watch-read-next aria-label="Next comprehension part" '+(index===1?'disabled':'')+'>Next</button></footer></section>';
  }
  ...
  function languageContentPage(page){
    ...
    const content=readLanguageContent(),loc=currentLocation(),items=itemsFor(content,page,loc),learner=readLearnerState(),index=languagePageIndex(page,loc,items),item=items[index],total=items.length,current=total?index+1:0,progress=total?Math.round((current/total)*100):0,level=Math.max(1,Number(learner.currentLevel)||1),step=Math.max(1,Number(learner.currentStep)||1),box=Math.max(1,Number(learner.currentBox)||1),objective=page==='language-letters'?'Recognize meaning, retrieve the form, and produce it accurately.':page==='language-voice'?'Discriminate the sound, map it to meaning, then produce it aloud.':page==='language-examine'?'Retrieve and apply what you learned with minimal scaffolding.':'Complete the current learning item with deliberate attention.',markup=item?(page==='language-letters'?renderVocabularyItem(item,index):page==='language-voice'?renderVoiceItem(item,index):page==='language-examine'?renderExamItem(item,index):renderGenericLanguageItem(item,index)):'<article class="language-learning-card empty"><div class="language-item-kicker"><span>Content</span><b>00</b></div><h2>No content yet</h2><p class="language-item-prompt">Use Content Control to add a content item.</p></article>';
    return '<section class="language-content-page language-item-process" data-language-content-page="'+esc(page)+'"><header class="language-page-head"><div class="language-page-copy"><small>'+esc(targetLanguage())+' · focused practice</small><h1>'+esc(routeTitle(page))+'</h1><p>'+esc(objective)+'</p></div><div class="language-session-position" aria-label="Current learning position"><span><small>Level</small><strong>'+level+'</strong></span><span><small>Step</small><strong>'+step+'</strong></span><span><small>Box</small><strong>'+box+'</strong></span></div></header><div class="language-progress-block"><div class="language-step-progress" role="progressbar" aria-label="Item progress" aria-valuemin="0" aria-valuemax="100" aria-valuenow="'+progress+'"><span style="width:'+progress+'%"></span></div><div class="language-progress-meta"><span>Item '+current+' of '+total+'</span><strong>'+progress+'%</strong></div></div>'+markup+'<footer class="language-item-navigation"><button type="button" class="btn btn-ghost" data-language-item-prev aria-label="Previous learning item" '+(index===0?'disabled':'')+'>Previous</button><span><strong>'+current+'</strong><small>of '+total+'</small></span><button type="button" class="btn btn-primary" data-language-item-next aria-label="Next learning item" '+(!total||index>=total-1?'disabled':'')+'>Next</button></footer></section>';
  }
